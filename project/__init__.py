...
snake_img = pygame.image.load('head1.png')
...
def start_game():
    canvas.fill(BLACK)
    font1 = large_font.render("Добро пожаловать в игру", True, BLUE)
    font2 = medium_font.render("Играть", True, BLACK, PURPLE)
    font3 = medium_font.render("Правила", True, BLACK, PURPLE)
    font4 = medium_font.render("Выйти", True, BLACK, PURPLE)
    ...
    font1_rect.center = (WINDOW_WIDTH/2, WINDOW_HEIGHT/2 - 100)
    font2_rect.center = (WINDOW_WIDTH/2 + 100, WINDOW_HEIGHT/2 + 50)
    font3_rect.center = (WINDOW_WIDTH/2 + 100, WINDOW_HEIGHT / 2 + 100)
    font4_rect.center = (WINDOW_WIDTH/2 + 100, WINDOW_HEIGHT/2 + 150)

    canvas.blit(font1, font1_rect)
    canvas.blit(font2, font2_rect)
    canvas.blit(font3, font3_rect)
    canvas.blit(font4, font4_rect)
...
def start_inst(start_font1, start_font1_rect):
    ...
    start_inst1 = small_font.render("Не врезайтесь в границу - умрёте", True, BLUE)
    start_inst2 = small_font.render("Ешьте яблоки, они дают очки", True, BLUE)
    start_inst3 = small_font.render("Не врезайтесь в себя", True, BLUE)
    start_inst4 = small_font.render("Удачи в  игре", True, BLUE)
    start_inst5 = medium_font.render("Назад", True, RED, YELLOW)
...
def gameover():
    ...
    font_gameover1 = large_font.render('Игра окончена', True, PURPLE)
    font_gameover2 = medium_font.render("Играть снова", True, GREEN, BLACK)
    font_gameover3 = medium_font.render("Выйти", True, RED, BLACK)
...
def game_paused():
    # canvas.fill(BLACK)
    paused_font1 = large_font.render("Пауза", True, RED)
    ...
    while True:
        ...
        while True:
            ...
            if snake_head_rect.colliderect(apple_rect):
                ...
                APPLE_Y = random.randrange(TOP_WIDTH, WINDOW_HEIGHT - 10, 10)
                snakelength += 1
                score = small_font.render("Очки:" + str(snakelength - 3), True, YELLOW)

            canvas.blit(score, (20, 10))
            pygame.draw.line(canvas, PURPLE, (0, TOP_WIDTH), (WINDOW_WIDTH, TOP_WIDTH))
